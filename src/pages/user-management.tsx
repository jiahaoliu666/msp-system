...
export default function UserManagement() {
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      ...
      // 檢查是否為系統管理員更改自己的角色
      if (currentUserEmail === editFormData.email && selectedUser?.role === '系統管理員' && editFormData.role !== '系統管理員') {
        const confirmChange = window.confirm('警告：若將自己的角色從系統管理員更改為其他角色，將無法再變更回系統管理員。確定要繼續嗎？');
        if (!confirmChange) {
          return;
        }
        // 如果確認要變更角色，設置一個標記
        const willNeedRefresh = true;
      }
      ...
      // 檢查架構師是否嘗試修改系統工程師的角色
      if (userRole === '架構師' && selectedUser?.role === '系統工程師' && selectedUser.role !== editFormData.role) {
        showToast('error', '架構師無法變更系統工程師的角色');
        return;
      }
        ...
        <>
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-gray-600 mt-1">管理所有用戶資料與權限設定</p>
            </div>
          ...
          <div className="mb-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              >
                ...
                {ROLE_CONFIG.getRoleOptions(userRole ?? undefined).map((option: RoleOption) => (
                      ...
                      <tr key={user.username} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-sm font-medium text-center">
                          <div className="flex justify-center space-x-2">
                            ...
                            {userRole !== '維運工程師' && (userRole === '系統管理員' || user.role !== '系統管理員') && (
                              <>
